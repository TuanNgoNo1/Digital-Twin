...
public static class PageTwoSceneSetup
{
    ...
    {
        controller.FindProperty("boardPreviewMaterial").objectReferenceValue =
            AssetDatabase.LoadAssetAtPath<Material>("Assets/Materials/NewServoWiringBoard_2D.mat");
        AssignGameObjectArray(controller.FindProperty("demoWirePrefabs"), new[]
        {
    ...
    private static void AssignGameObjectArray(SerializedProperty property, string[] paths)
    {
        property.arraySize = paths.Length;
        for (int i = 0; i < paths.Length; i++)
        {
            property.GetArrayElementAtIndex(i).objectReferenceValue = AssetDatabase.LoadAssetAtPath<GameObject>(paths[i]);
        }
    }
