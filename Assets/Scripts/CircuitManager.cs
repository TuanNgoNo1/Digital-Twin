...
public class CircuitManager : MonoBehaviour
{
    ...
    private const float SceneAuthoredJackDiameterPixels = 14f;
    ...
    private GameObject motorSocketLabelRoot;
    private readonly List<MotorSocketLabelBinding> motorSocketLabels =
        new List<MotorSocketLabelBinding>();
    private SocketPoint hoveredSocket;
    private SceneAuthoredWiringBoard sceneAuthoredWiringBoard;
    ...
    private sealed class MotorSocketLabelBinding
    {
    ...
    private void Update()
    {
        ...
        UpdateMotorSocketLabelPositions();
    }

    private void UpdateSocketHover()
    {
        SocketPoint nextHoveredSocket = null;
        bool canHover = connectBySocketClick && initialized && !popupVisible &&
            !systemUnlocked && visibleStepIndex == currentStepIndex;
        if (canHover)
        {
            ...
            if (!IsScreenPointInside(resetStepButton, screenPosition) &&
                !IsScreenPointInside(previousStepButton, screenPosition) &&
                !IsPointerOverStepNavigation(screenPosition))
            {
                nextHoveredSocket = FindSocketAtScreenPosition(screenPosition);
            }
        }

        if (hoveredSocket == nextHoveredSocket)
            return;

        if (hoveredSocket != null)
            hoveredSocket.SetPointerHover(false);
        hoveredSocket = nextHoveredSocket;
        if (hoveredSocket != null)
            hoveredSocket.SetPointerHover(true);
    }
    ...
    public void InitializeGame()
    {
        ...
        CreateMotorSocketLabels(Camera.main);
    ...
    {
        sceneAuthoredWiringBoard = FindFirstObjectByType<SceneAuthoredWiringBoard>(
            FindObjectsInactive.Include);
        if (sceneAuthoredWiringBoard != null && sceneAuthoredWiringBoard.boardRenderer != null)
        {
            ApplySceneAuthoredServoBoardLayout();
            return;
        }
        ...
        Dictionary<string, Vector2> socketPixels = new Dictionary<string, Vector2>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            { "X4", new Vector2(137f, 410.5f) },
            { "X3", new Vector2(137f, 473.5f) },
            { "X2", new Vector2(137f, 537.5f) },
            { "X1", new Vector2(137f, 600.5f) },
            { "X0", new Vector2(137f, 665.5f) },
            { "SS", new Vector2(137f, 730.5f) },
            { "Y4", new Vector2(409f, 418.5f) },
            { "Y3", new Vector2(409f, 486f) },
            { "Y2", new Vector2(408.5f, 554f) },
            { "Y1", new Vector2(408.65f, 624.13f) },
            { "Y0", new Vector2(408.51f, 689.15f) },
            { "+V4", new Vector2(537.5f, 427f) },
            { "+V3", new Vector2(537.5f, 502f) },
            { "+V2", new Vector2(537.5f, 578.5f) },
            { "+V1", new Vector2(539.2f, 656.6f) },
            { "+V0", new Vector2(538.6f, 731.8f) },
            ...
            { "Pin13", new Vector2(678.85f, 724.9f) },
            { "Pin14", new Vector2(678.75f, 782.2f) },
            { "oA", new Vector2(863.5f, 377f) },
            { "oB", new Vector2(865.5f, 458.5f) },
            { "oC", new Vector2(865.5f, 535f) },
            { "Motor_A", new Vector2(1071.3f, 562.1f) },
            { "Motor_B", new Vector2(1070.5f, 619.5f) },
            { "Motor_C", new Vector2(1071f, 676.7f) },
            { "Enc_A", new Vector2(1130f, 802f) },
            { "Enc_B", new Vector2(1130.5f, 885.5f) }
        };
        ...
        foreach (SocketPoint socket in FindObjectsByType<SocketPoint>(FindObjectsInactive.Include, FindObjectsSortMode.None))
        {
            ...
            socket.transform.position = surface.transform.TransformPoint(
                new Vector3(horizontal, vertical, 0f)) + surfaceNormal * 0.01f;
    ...
    private void ApplySceneAuthoredServoBoardLayout()
    {
        newBoardSurface = sceneAuthoredWiringBoard.boardRenderer.gameObject;
        newSocketVisualRoot = sceneAuthoredWiringBoard.jackVisualRoot;

        EnsureNonInteractiveSocketObstacles();
        FitSceneAuthoredBoardToWorkspace();

        GameObject legacyBoard = GameObject.Find("Board");
        if (legacyBoard != null)
            HideLegacyBoardModels(legacyBoard.transform.root, sceneAuthoredWiringBoard.boardRenderer);
        HideLegacyBoardOverlays();

        foreach (SocketPoint socket in sceneAuthoredWiringBoard.sockets)
        {
            if (socket == null || !sceneAuthoredWiringBoard.TryGetPoses(
                    socket,
            ...
            savedSocketPoses.Add(new SavedSocketPose
            {
                Socket = socket,
                OriginalPosition = operationPosition,
                OriginalRotation = operationRotation,
                WiringPosition = wiringPosition,
                WiringRotation = wiringRotation
            });
            socket.transform.SetPositionAndRotation(wiringPosition, wiringRotation);
        }

        newBoardSurface.SetActive(true);
        if (newSocketVisualRoot != null)
            newSocketVisualRoot.SetActive(true);
        Debug.Log($"[Circuit] Su dung bang noi day 2D da duoc cau hinh trong scene ({sceneAuthoredWiringBoard.sockets.Count} socket).");
    }

    private void EnsureNonInteractiveSocketObstacles()
    {
        if (newBoardSurface == null)
            return;
        if (newBoardSurface.GetComponentsInChildren<WireRoutingObstacle>(true).Length > 0)
            return;

        Camera mainCamera = Camera.main != null ? Camera.main : FindFirstObjectByType<Camera>();
        if (mainCamera == null)
            return;
        Bounds bounds = sceneAuthoredWiringBoard.boardRenderer.bounds;
        Vector3 right = mainCamera.transform.right.normalized;
        Vector3 up = mainCamera.transform.up.normalized;
        Vector3 normal = -mainCamera.transform.forward.normalized;
        float width = ProjectedBoundsSize(bounds, right);
        float height = ProjectedBoundsSize(bounds, up);
        Dictionary<string, Vector2> normalizedPositions = new Dictionary<string, Vector2>
        {
            { "X2", new Vector2(137f / 1448f - 0.5f, 0.5f - 537.5f / 1086f) },
            { "Y2", new Vector2(408.5f / 1448f - 0.5f, 0.5f - 554f / 1086f) },
            { "Y3", new Vector2(409f / 1448f - 0.5f, 0.5f - 486f / 1086f) },
            { "Y4", new Vector2(409f / 1448f - 0.5f, 0.5f - 418.5f / 1086f) },
            { "+V2", new Vector2(537.5f / 1448f - 0.5f, 0.5f - 578.5f / 1086f) },
            { "+V3", new Vector2(537.5f / 1448f - 0.5f, 0.5f - 502f / 1086f) },
            { "+V4", new Vector2(537.5f / 1448f - 0.5f, 0.5f - 427f / 1086f) },
            { "Pin16", new Vector2(678f / 1448f - 0.5f, 0.5f - 672f / 1086f) }
        };

        GameObject root = new GameObject("NonInteractiveSocketObstacles_Runtime");
        root.transform.SetParent(newBoardSurface.transform, false);
        foreach (KeyValuePair<string, Vector2> entry in normalizedPositions)
        {
            ...
            obstacle.transform.position = bounds.center +
                right * (entry.Value.x * width) +
                up * (entry.Value.y * height) +
                normal * 0.012f;
            obstacle.AddComponent<WireRoutingObstacle>();
        }
    }

    private void FitSceneAuthoredBoardToWorkspace()
    {
        Camera mainCamera = Camera.main != null ? Camera.main : FindFirstObjectByType<Camera>();
        Transform surface = sceneAuthoredWiringBoard.boardRenderer.transform;
        if (mainCamera == null || surface == null)
            return;

        Vector3 right = mainCamera.transform.right.normalized;
        Vector3 up = mainCamera.transform.up.normalized;
        Vector3 normal = -mainCamera.transform.forward.normalized;
        Bounds oldBounds = sceneAuthoredWiringBoard.boardRenderer.bounds;
        Vector3 oldCenter = oldBounds.center;
        float oldWidth = ProjectedBoundsSize(oldBounds, right);
        float oldHeight = ProjectedBoundsSize(oldBounds, up);
        if (oldWidth <= 0.000001f || oldHeight <= 0.000001f)
            return;

        float textureAspect = oldWidth / oldHeight;
        Material material = sceneAuthoredWiringBoard.boardRenderer.sharedMaterial;
        Texture texture = material != null
            ? (material.HasProperty("_BaseMap") ? material.GetTexture("_BaseMap") : material.mainTexture)
            : null;
        ...
        if (!TryFitBoardInsideModelPanel(
                mainCamera,
                oldCenter,
                textureAspect,
                ref newCenter,
        ...
        surface.position = newCenter;
        surface.localScale = new Vector3(
            surface.localScale.x * scaleX,
            surface.localScale.y * scaleY,
            surface.localScale.z);

        for (int i = 0; i < sceneAuthoredWiringBoard.sockets.Count; i++)
        {
            SocketPoint socket = sceneAuthoredWiringBoard.sockets[i];
            if (socket == null || i >= sceneAuthoredWiringBoard.wiringPositions.Count)
                continue;

            Vector3 authoredPosition = sceneAuthoredWiringBoard.wiringPositions[i];
            float normalizedX = Vector3.Dot(authoredPosition - oldCenter, right) / oldWidth;
            float normalizedY = Vector3.Dot(authoredPosition - oldCenter, up) / oldHeight;
            bool hasExactMotorPixel = false;
            if (texture != null && texture.width > 0 && texture.height > 0 &&
                TryGetScreenCalibratedSocketPixel(socket.socketID, out Vector2 calibratedPixel))
            {
                normalizedX = calibratedPixel.x / texture.width - 0.5f;
                normalizedY = 0.5f - calibratedPixel.y / texture.height;
                hasExactMotorPixel = true;
            }
            Vector3 fittedPosition = hasExactMotorPixel
                ? surface.TransformPoint(new Vector3(normalizedX, normalizedY, 0f)) + normal * 0.01f
                : newCenter +
                  right * (normalizedX * newWidth) +
                  up * (normalizedY * newHeight) +
                  normal * 0.01f;
            sceneAuthoredWiringBoard.wiringPositions[i] = fittedPosition;
            socket.transform.SetPositionAndRotation(fittedPosition, surface.rotation);

            if (newSocketVisualRoot == null)
                continue;
            Transform jack = newSocketVisualRoot.transform.Cast<Transform>()
                .FirstOrDefault(candidate => candidate.name.StartsWith(
                    $"Jack_{socket.socketID}_",
                    StringComparison.OrdinalIgnoreCase));
            if (jack == null)
                continue;
            jack.position = fittedPosition;
            jack.rotation = Quaternion.LookRotation(
                (mainCamera.transform.position - fittedPosition).normalized,
                mainCamera.transform.up);
            if (TryGetRendererBounds(jack.gameObject, out Bounds jackBounds))
            {
                float currentDiameter = Mathf.Max(
                    ProjectedBoundsSize(jackBounds, right),
                    ProjectedBoundsSize(jackBounds, up));
                float targetDiameter = ScreenPixelsToWorldSize(
                    mainCamera,
                    fittedPosition,
                    SceneAuthoredJackDiameterPixels);
                if (currentDiameter > 0.000001f && targetDiameter > 0f)
                    jack.localScale *= targetDiameter / currentDiameter;
            }

            CenterJackVisual(jack, fittedPosition, mainCamera);
        }
    }
    ...
    private void CreateNewSocketVisuals(Camera mainCamera)
    {
                     ...
                     FindObjectsSortMode.None))
        {
            ...
            // Point local +Z at the perspective camera so the jack's depth
            // remains concentric instead of projecting sideways.
            visual.transform.rotation = Quaternion.LookRotation(
                (mainCamera.transform.position - socket.transform.position).normalized,
                mainCamera.transform.up);
    ...
    private void CreateMotorSocketLabels(Camera camera)
    {
        if (camera == null || newSocketVisualRoot == null)
            return;

        if (motorSocketLabelRoot != null)
            Destroy(motorSocketLabelRoot);

        motorSocketLabels.Clear();
        motorSocketLabelRoot = new GameObject(
            "MotorSocketLabels",
            typeof(RectTransform),
            typeof(Canvas),
            typeof(CanvasScaler));
        motorSocketLabelRoot.transform.SetParent(transform, false);

        Canvas canvas = motorSocketLabelRoot.GetComponent<Canvas>();
        canvas.renderMode = RenderMode.ScreenSpaceOverlay;
        canvas.sortingOrder = 4700;

        CanvasScaler scaler = motorSocketLabelRoot.GetComponent<CanvasScaler>();
        scaler.uiScaleMode = CanvasScaler.ScaleMode.ScaleWithScreenSize;
        scaler.referenceResolution = new Vector2(1920f, 1080f);
        scaler.screenMatchMode = CanvasScaler.ScreenMatchMode.MatchWidthOrHeight;
        scaler.matchWidthOrHeight = 0.5f;

        string[] socketIds = { "Motor_A", "Motor_B", "Motor_C" };
        foreach (string socketId in socketIds)
        {
            SocketPoint socket = FindObjectsByType<SocketPoint>(
                    FindObjectsInactive.Include,
                    FindObjectsSortMode.None)
                .FirstOrDefault(candidate =>
                    candidate != null &&
                    string.Equals(candidate.socketID, socketId, StringComparison.OrdinalIgnoreCase));
            if (socket == null)
                continue;

            GameObject labelBackgroundObject = new GameObject(
                "Label_" + socketId,
                typeof(RectTransform),
                typeof(CanvasRenderer),
                typeof(Image));
            labelBackgroundObject.transform.SetParent(motorSocketLabelRoot.transform, false);

            RectTransform backgroundRect = labelBackgroundObject.GetComponent<RectTransform>();
            backgroundRect.anchorMin = new Vector2(0.5f, 0.5f);
            backgroundRect.anchorMax = new Vector2(0.5f, 0.5f);
            backgroundRect.pivot = new Vector2(0f, 0.5f);
            backgroundRect.sizeDelta = new Vector2(84f, 25f);

            Image background = labelBackgroundObject.GetComponent<Image>();
            background.color = new Color32(255, 255, 255, 248);
            background.sprite = GetSocketLabelBackgroundSprite();
            ...
            labelObject.transform.SetParent(labelBackgroundObject.transform, false);
            ...
            label.color = GetMotorSocketLabelColor(socket.acceptColor);
            label.outlineColor = new Color32(255, 255, 255, 210);
            label.outlineWidth = 0.14f;
            label.margin = Vector4.zero;
            label.raycastTarget = false;

            Vector2 preferredTextSize = label.GetPreferredValues(label.text);
            backgroundRect.sizeDelta = new Vector2(
                Mathf.Ceil(preferredTextSize.x) + 10f,
                Mathf.Ceil(preferredTextSize.y) + 4f);

            RectTransform textRect = label.rectTransform;
            StretchRect(textRect);
            motorSocketLabels.Add(new MotorSocketLabelBinding
            {
                Socket = socket,
                Rect = backgroundRect,
                ScreenOffset = new Vector2(14f, 0f)
            });
        }

        SocketPoint ground5VSocket = FindObjectsByType<SocketPoint>(
                FindObjectsInactive.Include,
                FindObjectsSortMode.None)
            .FirstOrDefault(candidate =>
                candidate != null &&
                string.Equals(candidate.socketID, "GND_5V", StringComparison.OrdinalIgnoreCase));
        if (ground5VSocket != null)
        {
            GameObject labelObject = new GameObject(
                "Label_GND_5V",
                typeof(RectTransform),
                typeof(CanvasRenderer),
                typeof(TextMeshProUGUI));
            labelObject.transform.SetParent(motorSocketLabelRoot.transform, false);

            TextMeshProUGUI label = labelObject.GetComponent<TextMeshProUGUI>();
            label.text = "GND_5V";
            // Match the large red "5VDC" heading baked into the board artwork.
            label.fontSize = 24f;
            label.fontStyle = FontStyles.Bold;
            label.fontWeight = FontWeight.Black;
            ...
            label.outlineColor = new Color32(255, 255, 255, 220);
            ...
            motorSocketLabels.Add(new MotorSocketLabelBinding
            {
                Socket = ground5VSocket,
                Rect = rect,
                ScreenOffset = new Vector2(19f, 24f)
            });
        }
    ...
    private static Color GetMotorSocketLabelColor(WireColor wireColor)
    {
        switch (wireColor)
        {
            case WireColor.Red:
                return new Color(1f, 0.018f, 0.035f, 1f);
            case WireColor.Yellow:
                return new Color32(180, 83, 9, 255);
            case WireColor.Black:
                return new Color(0.075f, 0.085f, 0.11f, 1f);
            case WireColor.Green:
                return new Color(0.015f, 0.9f, 0.12f, 1f);
            case WireColor.Blue:
                return new Color(0.015f, 0.38f, 1f, 1f);
            default:
                return new Color32(24, 52, 94, 255);
        }
    }

    private void UpdateMotorSocketLabelPositions()
    {
        if (motorSocketLabelRoot == null)
            return;

        bool boardVisible = newSocketVisualRoot != null &&
            newSocketVisualRoot.activeInHierarchy &&
            (newBoardSurface == null || newBoardSurface.activeInHierarchy);
        if (motorSocketLabelRoot.activeSelf != boardVisible)
            motorSocketLabelRoot.SetActive(boardVisible);
        if (!boardVisible)
            return;

        Camera camera = Camera.main;
        RectTransform canvasRect = motorSocketLabelRoot.transform as RectTransform;
        if (camera == null || canvasRect == null)
            return;

        foreach (MotorSocketLabelBinding binding in motorSocketLabels)
        {
            if (binding.Socket == null || binding.Rect == null)
                continue;

            Vector3 screen = camera.WorldToScreenPoint(binding.Socket.transform.position);
            if (screen.z <= camera.nearClipPlane)
            {
            ...
            binding.Rect.gameObject.SetActive(true);
            ...
            {
                binding.Rect.anchoredPosition = canvasPoint + binding.ScreenOffset;
            }
        }
    }
    ...
    private static void CenterJackVisual(
        Transform jack,
        Vector3 socketPosition,
        Camera camera)
    {
        if (jack == null || camera == null ||
            !TryGetRendererBounds(jack.gameObject, out Bounds bounds))
            return;

        Vector3 socketScreen = camera.WorldToScreenPoint(socketPosition);
        Vector3 visualScreen = camera.WorldToScreenPoint(bounds.center);
        Vector3 alignedCenter = camera.ScreenToWorldPoint(
            new Vector3(socketScreen.x, socketScreen.y, visualScreen.z));
        jack.position += alignedCenter - bounds.center;
    }

    private static bool TryGetScreenCalibratedSocketPixel(string socketId, out Vector2 pixel)
    {
        switch (socketId)
        {
            // Screen-calibrated coordinates for the angled perspective board.
            case "24VDC": pixel = new Vector2(121.86f, 234.59f); return true;
            case "GND_24V": pixel = new Vector2(249.03f, 234.59f); return true;
            case "5VDC": pixel = new Vector2(395.73f, 234.59f); return true;
            case "GND_5V": pixel = new Vector2(520.04f, 234.59f); return true;
            case "X4": pixel = new Vector2(142.39f, 413.17f); return true;
            case "X3": pixel = new Vector2(142.43f, 474.81f); return true;
            case "X2": pixel = new Vector2(142.32f, 537.44f); return true;
            case "X1": pixel = new Vector2(142.36f, 599.08f); return true;
            case "X0": pixel = new Vector2(142.38f, 662.69f); return true;
            case "SS": pixel = new Vector2(142.40f, 726.29f); return true;
            case "Pin11": pixel = new Vector2(671.75f, 342.22f); return true;
            case "Pin12": pixel = new Vector2(671.69f, 402.89f); return true;
            case "Pin9": pixel = new Vector2(671.77f, 461.60f); return true;
            case "Pin10": pixel = new Vector2(671.79f, 524.72f); return true;
            case "Pin15": pixel = new Vector2(671.74f, 609.36f); return true;
            case "Pin16": pixel = new Vector2(671.79f, 669.05f); return true;
            case "Pin13": pixel = new Vector2(672.61f, 720.81f); return true;
            case "Pin14": pixel = new Vector2(672.50f, 776.88f); return true;
            case "+V1": pixel = new Vector2(535.87f, 653.98f); return true;
            case "+V0": pixel = new Vector2(535.25f, 727.56f); return true;
            case "Y1": pixel = new Vector2(408.20f, 622.20f); return true;
            case "Y0": pixel = new Vector2(408.02f, 685.83f); return true;
            case "Motor_A": pixel = new Vector2(1071.3f, 562.1f); return true;
            case "Motor_B": pixel = new Vector2(1070.5f, 619.5f); return true;
            case "Motor_C": pixel = new Vector2(1071f, 676.7f); return true;
            case "oA": pixel = new Vector2(853.18f, 380.39f); return true;
            case "oB": pixel = new Vector2(855.26f, 460.13f); return true;
            case "oC": pixel = new Vector2(855.23f, 534.99f); return true;
            case "Enc_A": pixel = new Vector2(1114.05f, 796.25f); return true;
            case "Enc_B": pixel = new Vector2(1114.45f, 877.96f); return true;
            default: pixel = default; return false;
        }
    }
    ...
    {
        if (newBoardMaterial != null && sceneAuthoredWiringBoard == null)
            Destroy(newBoardMaterial);
        if (newBoardSurface != null && sceneAuthoredWiringBoard == null)
            Destroy(newBoardSurface);
        if (newSocketVisualRoot != null && sceneAuthoredWiringBoard == null)
            Destroy(newSocketVisualRoot);
    }
    ...
    private void CreateReplacementWireInstances()
    {
        ...
        foreach (GameObject stepRoot in stepRoots)
        {
            foreach (WireBody wire in GetStepWires(stepRoot))
            {
                ...
                // Build each connected cable from its live socket anchors so
                // both ends and its length follow the replacement board.
    ...
    private static Color GetProceduralWireColor(WireBody wire)
    {
        ...
        {
            case WireColor.Red: return new Color(1f, 0.018f, 0.035f, 1f);
            case WireColor.Black: return new Color(0.075f, 0.085f, 0.11f, 1f);
            case WireColor.Yellow: return new Color(1f, 0.84f, 0.015f, 1f);
            case WireColor.Green: return new Color(0.015f, 0.9f, 0.12f, 1f);
            case WireColor.Blue: return new Color(0.015f, 0.38f, 1f, 1f);
