...
public class PageTwoPartsController : MonoBehaviour
{
    ...
    private readonly PartEntry[] parts =
    {
        ...
        new PartEntry("Bảng cắm dây", "BlueServoBoardOverlay")
    };
    ...
    [Header("Preview model assets")]
    [SerializeField] private Material boardPreviewMaterial;
    [SerializeField] private GameObject[] demoWirePrefabs;
    [SerializeField, HideInInspector] private GameObject jack35Prefab;
    [SerializeField, HideInInspector] private Material[] wireMaterials;
    [SerializeField, HideInInspector] private Material[] jackBodyMaterials;
    ...
    private void SelectPart(int index, RectTransform sourceButton)
    {
        ...
        SetCameraTarget(targetBounds, GetDetailPadding(index));
    ...
    private IEnumerator AnimateCameraOnly()
    {
        ...
        while (elapsed < animationDuration)
        {
            ...
            float t = Mathf.SmoothStep(0f, 1f, Mathf.Clamp01(elapsed / animationDuration));
            previewCamera.transform.position = Vector3.Lerp(cameraStartPosition, cameraTargetPosition, t);
            previewCamera.transform.rotation = Quaternion.Slerp(cameraStartRotation, cameraTargetRotation, t);
            previewCamera.orthographicSize = Mathf.Lerp(cameraStartSize, cameraTargetSize, t);
            yield return null;
        }
        transition = null;
    ...
    private void SetCameraTarget(Bounds bounds, float padding)
    {
        ...
        cameraStartRotation = previewCamera.transform.rotation;
        cameraStartSize = previewCamera.orthographicSize;
        orbitYaw = -12f;
        orbitPitch = 8f;
        cameraTargetPosition = CalculateCameraPosition(bounds, padding);
        cameraTargetRotation = Quaternion.LookRotation(bounds.center - cameraTargetPosition, Vector3.up);
    ...
    private static float GetDetailPadding(int partIndex)
    {
        // Cụm dây và mặt bảng phẳng cần khung hình sát hơn các model có chiều sâu.
        if (partIndex == 5)
            return 1.22f;
        if (partIndex == 6)
            return 1.12f;
    ...
    private void HandleResolutionChange()
    {
        ...
        orbitDistance = CalculateCameraDistance(
            activeViewBounds,
            isShowingDetails ? GetDetailPadding(selectedPartIndex) : 1.18f);
        UpdateOrbitCamera();
    ...
    {
        if (modelRoot == null || boardPreviewMaterial == null)
        {
            Debug.LogWarning("[PageTwoPartsController] Chua gan material bang noi day mau xanh.");
            return;
        }

        Transform board = modelRoot.GetComponentsInChildren<Transform>(true)
        ...
        if (board == null)
        {
            Debug.LogWarning("[PageTwoPartsController] Khong tim thay Board de thay giao dien mau xanh.");
            return;
        }
        ...
        // Mặt bảng mới nằm trước cụm linh kiện cũ nhưng vẫn sau các thiết bị trên bàn.
        overlay.transform.localPosition = new Vector3(0f, 0.075f, 0.20f);
        overlay.transform.localRotation = Quaternion.identity;
        overlay.transform.localScale = new Vector3(0.46f, 0.345f, 1f);
    ...
    {
        if (demoWirePrefabs == null || demoWirePrefabs.Length < 3)
        {
            Debug.LogWarning("[PageTwoPartsController] Chua gan du ba model Wires 3, dung day du phong.");
            CreateLegacyDemoWires();
            return;
        }

        Bounds modelBounds = CalculateBounds(new[] { modelRoot });
        GameObject group = new GameObject("Page2DemoWires");
        group.transform.SetParent(modelRoot, false);
        group.transform.localPosition = Vector3.zero;
        group.transform.localRotation = Quaternion.identity;
        group.transform.localScale = Vector3.one;

        float targetHeight = Mathf.Max(modelBounds.size.y * 0.29f, 0.12f);
        float horizontalGap = Mathf.Max(modelBounds.size.x * 0.045f, 0.025f);
        float wireSpacing = Mathf.Max(modelBounds.size.x * 0.035f, 0.022f);
        float wireWidth = Mathf.Max(modelBounds.size.x * 0.008f, 0.004f);
        float centerY = modelBounds.center.y + modelBounds.size.y * 0.055f;
        float z = modelBounds.center.z;
        for (int i = 0; i < 3; i++)
        {
            GameObject prefab = demoWirePrefabs[i];
            if (prefab == null)
                continue;

            Renderer sourceRenderer = prefab.GetComponentInChildren<Renderer>(true);
            GameObject wire = new GameObject("DemoWire_" + (i + 1) + "_" + prefab.name, typeof(LineRenderer));
            wire.transform.SetParent(group.transform, false);
            ...
            line.sharedMaterial = CreateStraightWireMaterial(sourceRenderer, i);
            line.startColor = Color.white;
            line.endColor = Color.white;

            float x = modelBounds.max.x + horizontalGap + wireSpacing * i;
            line.SetPosition(0, new Vector3(x, centerY + targetHeight * 0.5f, z));
            line.SetPosition(1, new Vector3(x, centerY - targetHeight * 0.5f, z));
        }
    }

    private Material CreateStraightWireMaterial(Renderer sourceRenderer, int colorIndex)
    {
        Color[] fallbackColors =
        {
            new Color(0.92f, 0.02f, 0.04f, 1f),
            new Color(0.65f, 0.42f, 0.02f, 1f),
            new Color(0.06f, 0.07f, 0.08f, 1f)
        };
        Color color = fallbackColors[Mathf.Clamp(colorIndex, 0, fallbackColors.Length - 1)];
        ...
        Material material = new Material(shader) { name = $"Page2StraightWire_{colorIndex}" };
    ...
    private void CreateLegacyDemoWires()
    {
        Bounds modelBounds = CalculateBounds(new[] { modelRoot });
        GameObject group = new GameObject("Page2DemoWires");
